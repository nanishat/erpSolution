            ...
            return (
              <tr key={entry.id} className="border-t border-border">
                ...
                <td className="px-3 py-2 text-right whitespace-nowrap">
                  {entry.status === "DRAFT" &&
                    (entry.invoice ? (
                      // Neither Edit nor Post is offered here: both are
                      // rejected at the service layer for an invoice-linked
                      // entry (editing would diverge its lines from
                      // Invoice.subtotal/lines, posting would skip the
                      // Invoice status/Partner balance update) — this link
                      // is just reflecting that, not the actual enforcement,
                      // so it points at the real Invoice/Vendor Bill detail
                      // page where those actions live.
