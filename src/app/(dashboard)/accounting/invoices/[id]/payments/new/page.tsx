...
export default async function RecordInvoicePaymentPage({
  params,
}: {
  ...
  const detailHref = `/accounting/invoices/${invoice.id}`;
    ...
    return (
      <div className="space-y-4">
        <p className="text-sm text-muted-foreground">
          Invoice {invoice.invoiceNumber} is {invoice.status} and cannot receive a payment.
        </p>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Against invoice {invoice.invoiceNumber} — {invoice.partner.name}
        </p>
      </div>
      <RecordPaymentForm
