...
export default async function CustomerInvoiceDetailPage({
  ...
  // Tax can only be attached before the invoice's JournalEntry is posted —
  // same gate as JournalEntryDetailPage (see its own doc comment).
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 rounded-lg border border-border p-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div>
          ...
          <div className="text-sm">{invoice.sector ?? "—"}</div>
        </div>
        {invoice.notes && (
      ...
      <div className="flex items-center justify-between">
        ...
        <Button asChild variant="outline" size="sm">
          <Link href={`${BASE_PATH}/${invoice.id}/print`} target="_blank">
            Print
          </Link>
        </Button>
      </div>
